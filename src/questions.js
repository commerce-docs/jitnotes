...
export default function askQuestions() {
  ...
  const validVersion = new RegExp(/^(?:\d|[1-9]\d)\.(?:[1-9]\d)\.(?:[1-9])$/);
  ...
  const questions = [
    ...
    },
    {
      name: 'releaseVersion',
      type: 'input',
      message: chalk.green(`Enter the ${chalk.yellow('release version')} (13.1.0):`),
      default: '13.1.0',
    ...
    },
    {
      ...
      type: 'input',
      message: chalk.green(`Enter the ${chalk.yellow('previous version')} released (13.0.0):`),
      default: '13.0.0',
      validate: thisAnswer => validVersion.test(thisAnswer) ? true : 'Please enter a valid version number: XX.X.X',
      when: priorAnswers => priorAnswers['jiraProject'] === 'PWA',
    },
